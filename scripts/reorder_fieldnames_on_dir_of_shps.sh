...
if [ $# -ne 3 ]; then
    echo "USAGE: ./reorder_fieldnames_on_dir_of_shps.sh <in_dir_path> <out_dir_path> <fieldname_options>"
...
for i in $(ls ${in_dir_of_shps}/*.shp); do
  ...
  python ../ogrtools.py --in_file ${i} --out_file ${o} --function reorder --arguments fieldname_options
done