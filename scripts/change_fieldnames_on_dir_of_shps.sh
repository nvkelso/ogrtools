...
if [ $# -ne 3 ]; then
    echo "USAGE: ./change_fieldnames_on_dir_of_shps.sh <in_dir_path> <out_dir_path> <fieldname_options>"
    echo "NOTE: Requires fieldname_options list in format of: Fieldname1 as NewFieldName1, Fieldname2 as NewFieldName2"
    exit 1
fi
...
for i in $(ls ${in_dir_of_shps}/*.shp); do
  ...
  python ../ogrtools.py --in_file ${i} --out_file ${o} --function rename --arguments fieldname_options
done